# Create an Account

<div class="plan-badges"><span class="plan-badge plan-normal">Normal</span><span class="plan-badge plan-pro">Pro</span><span class="plan-badge plan-team">Team</span></div>

Create a LanguageCheck account when you want to run checks, save your history, use words, or join a team.

## Steps

1. Open LanguageCheck and select **Sign up**.
2. Register with email and password, or continue with Google.
3. If you use email and password, confirm your email when you receive the verification message.
4. Continue with account setup when LanguageCheck asks for your data location and legal confirmations.

## Notes and Limitations

Google users may already be authenticated after sign-up. Email/password users may need to verify their email before all account features are enabled.

## Related Articles

- [Choose where your uploaded data is stored](data-location.md)
- [Complete your account setup](complete-account-setup.md)
- [Sign in with email, password, or Google](sign-in.md)

